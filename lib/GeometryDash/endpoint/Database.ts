...
export class GetGJScores20 extends GJEndpoint {
  ...
  static decode(str: string): T.User[] {
    ...
    return str
      .split('|')
      .filter((s) => s.length != 0)
      .map((item) => this.#serializer.decode(item));
  }
...
export class GetGJDailyLevel extends GJEndpoint {
  ...
  static encode(obj: {
    ...
    const list: (number | string)[] = [obj.index, obj.timeLeft];
    if (obj.rewards) {
  ...
  static async call(state: T.State, type: 0 | 1 | 2 = 0) {
    ...
    const rand_char = generateRS(5);
    const chk =
      rand_char + B64urlEnc(XOR(rand_int.toString(), key.chestRewards));
...
export class GetGJGauntlets21 extends GJEndpoint {
  ...
  static encode(obj: T.GauntletInfo[]) {
    return (
      obj.map((i) => this.#serializer.encode(i)).join('|') +
      '#' +
      this.hash(obj)
    );
  }
...
export class GetGJLevelLists extends GJEndpoint {
  static #serializer = TR.DelimObj<T.LevelList>(
    T.LevelListM,
    T.LevelListSM,
    ':'
  );
...
export class GetGJChallenges extends GJEndpoint {
  static #helper = TR.DelimArr<T.GetGJChallenges>(
    T.GetGJChallengesM,
    T.GetGJChallengesSM,
    ':'
  );
  ...
  static decode(ori: string): any {
    ...
    if (!b64_str || !hash) throw new Error('Unexpected response');

    const [rand, enc_str] = splitPos(b64_str, 5);
    if (!rand || !enc_str) throw new Error('Unexpected response');
...
export class GetGJRewards extends GJEndpoint {
  static #helper = TR.DelimArr<T.GetGJRewards>(
    T.GetGJRewardsM,
    T.GetGJRewardsSM,
    ':'
  );
