...
export class GeometryDash {
  ...
  getGJScores = (type: 'top' | 'friends' | 'relative' | 'creators', count?: number) => DB.GetGJScores20.call(this.state, type, count)
  getGJUserInfo = (targetAccountID: number) => DB.GetGJUserInfo20.call(this.state, targetAccountID)
  ...
  deleteGJLevelUser = (levelID: number) => DB.DeleteGJLevelUser20.call(this.state, levelID)
  downloadGJLevel = (levelID: number) => DB.DownloadGJLevel22.call(this.state, levelID)
  getGJDailyLevel = (type: 0 | 1 | 2  = 0) => DB.GetGJDailyLevel.call(this.state, type)
  ...
  //
  getGJLevelLists = (str: string, type: number, page: number) => DB.GetGJLevelLists.call(this.state, str, type, page)
  //
